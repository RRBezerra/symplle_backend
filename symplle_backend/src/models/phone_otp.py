...
class PhoneOTP(db.Model):
    ...
    def is_valid(self):
        ...
        # Verificar se expires_at tem timezone info
        if self.expires_at.tzinfo is None:
            # Se não tiver timezone (naive), converter para aware com UTC
            expires_at_aware = self.expires_at.replace(tzinfo=timezone.utc)
        else:
            # Se já tiver timezone (aware), usar como está
            expires_at_aware = self.expires_at
            
        return not self.verified and now <= expires_at_aware
    ...
    def verify(self, code):
        ...
        if self.verified or now > expires_at_aware:
            return False
